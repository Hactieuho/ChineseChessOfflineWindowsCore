  ...
  while ((mv = Search2.MoveSort.NextRoot()) != 0) {
    if (Search.pos.MakeMove(mv)) {
      ...
      if (vl > vlBest) {
        ...
        // 7. 搜索到最佳着法时记录主要变例
          if ((Search2.wmvPvLine[0])&&(Search2.wmvPvLine[0] != mv)) //Them doan nay de chong chieu lap
          {
              if(g_dwSecondStr != 0)
                  g_dwThirdStr = g_dwSecondStr;
              g_dwSecondStr = MOVE_COORD(Search2.wmvPvLine[0]);
          }
...
uint32_t g_dwMoveStr;
uint32_t g_dwSecondStr;
uint32_t g_dwThirdStr;
